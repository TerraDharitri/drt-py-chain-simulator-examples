...
from dharitri_py_sdk.core import Token, TokenTransfer, Address
from dharitri_py_sdk.network_providers.transactions import TransactionOnNetwork
from dharitri_py_sdk.core import TokenManagementTransactionsFactory, TransactionsFactoryConfig, TransferTransactionsFactory
from dharitri_py_sdk.network_providers import ProxyNetworkProvider
...
def main():
    ...
    sender_bech32 = key.generate_public_key().to_address("drt").to_bech32()
    ...
    # generate enough blocks to pass an epoch and the DCDT feature to be enabled
    ...
    print(f"issuing a fungible DCDT token. Name: {token_name} and ticker: {token_ticker}")
    ...
    #the issue token transaction has a cost of 0.05 REWA, the value is set automatically by the create_transaction_for_issuing_fungible call
    issue_tx.nonce = provider.get_account(sender).nonce
    issue_tx.signature = b"dummy"
    tx_hash = provider.send_transaction(issue_tx)
    print(f"fungible DCDT issue tx hash: {tx_hash}")
    time.sleep(0.5)
    ...
    print(f"issued fungible DCDT token with name {token_name} and ticker {token_ticker} received the identifier: {token_identifier_string}\n")
    ...
    key = UserSecretKey.generate()
    receiver_bech32 = key.generate_public_key().to_address("drt").to_bech32()
    ...
    transfer_tx = tx_factory.create_transaction_for_dcdt_token_transfer(
        sender=sender,
        receiver=receiver,
    ...
    print(f"transfer DCDT tx hash: {tx_hash}\n")
    time.sleep(0.5)
